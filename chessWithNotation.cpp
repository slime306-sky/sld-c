...
void recordMove(int fromFile, int fromRank, int toFile, int toRank) {
    ...
    string pieceLetters = "KQRNBP";
    char pieceLetter = pieceLetters[selectedPiece % 6];
    if (fromFile == toFile && fromRank == toRank) return;

    if (selectedPiece % 6 == 5) {
        move = (board[toRank][toFile] != -1) ? from[0] + string("x") + to : to;
    }
    else {
        move = pieceLetter + to;
    }
    moveHistory.push_back(move);
    cout << (moveHistory.size() / 2 + 1) << (isWhiteTurn ? ". " : "... ") << move << endl;
