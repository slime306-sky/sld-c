...
class handelAll : public gameUi, public gameLogic {
...
public:
	...
	void draggingWhen(SDL_Renderer* renderer) {
		int file = draggingX / SQUARE_SIZE;
		int rank = 7 - (draggingY / SQUARE_SIZE);
...
int main() {
	...
	hendeler.loadTextures(renderer);
	...
	while (running) {
		...
		hendeler.setColor(renderer, BLACK_COLOR);
		SDL_RenderClear(renderer);
		hendeler.drawBoard(renderer);
		hendeler.renderPieces(renderer);
		hendeler.draggingWhen(renderer);

		SDL_RenderPresent(renderer);
	}

	for (int i = 0; i < 12; i++) {
		if (hendeler.textures[i]) SDL_DestroyTexture(hendeler.textures[i]);
	}
